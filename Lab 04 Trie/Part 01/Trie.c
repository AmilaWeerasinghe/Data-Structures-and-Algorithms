...
void insertNode(TrieNode* root,char* word){
  int len=strlen(word);
	...
	for(i=0;i<strlen(word);i++){
      ...
      nodes++;//each time a node is created increment the number of nodes
...
bool findSuggestions(TrieNode *root,const  char *word)
{
    ...
    for (i = 0; i < len; i++){
      //find the inde for a given letter    
        index = word[i]- 'a';
   ...
   int main(int argc, char *argv[]){
    char fileName[100];
      ...
      printf("Size of struct: %ld\n", sizeof(struct trienode)); 
   		...
   		printf("file name = %s\n",argv[1]);
      printf("Size of struct: %ld", sizeof(struct trienode)); 
    ...
    printf("time taken to store the data in micro seconds:\n");
    printf("%f\n",mseconds);


  fclose(fp);
  //printf("number of node : %s\n",nodes );
    
    //successfully inserted now we need to find nd print suggestions
  char key[70];//to store input from user

  while(1){

    printf("Type a word  to suggest :");
    scanf("%s",key);
    printf(" ");
        start = clock();
